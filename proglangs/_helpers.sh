...
# Production mode: null or unset, debug mode: any other value
DEBUG=

declare -A MESSAGES=(
  [en.UTF-8.empty_parameter]="Error! Parameter {i} is empty!"
  [en.UTF-8.enter]="<Enter>"
  [en.UTF-8.erroneous_names]="Error! Some name are not packages! Please check your script!{NL}{NL}Erroneous names:"
  ...
  [en.UTF-8.missing_commands]="Error! Some commands are not available! Please install to continue!{NL}{NL}Missing commands:"
  [en.UTF-8.missing_filename]="Error! File name is not provided!"
  [en.UTF-8.missing_packages]="Error! Some packages are not installed! Please install to continue!{NL}{NL}Missing packages:"
  ...
  [lt_LT.UTF-8.erroneous_names]="Klaida! Klaidingi paketų pavadinimai! Patikrinkite savo skriptą!{NL}{NL}Klaidingi pavadinimai:"
  ...
  [lt_LT.UTF-8.missing_commands]="Klaida! Nėra reikalingų komandų! Įdiekite, kad galėtute tęsti!{NL}{NL}Trūkstamos komandos:"
  [lt_LT.UTF-8.missing_filename]="Klaida! Nepateiktas failo pavadinimas!"
  [lt_LT.UTF-8.missing_packages]="Klaida! Neįdiegti reikalingi paketai! Įdiekite, kad galėtute tęsti!{NL}{NL}Trūkstami paketai:"
...
check_command() (
  ...
  (( ${#NOT_COMMANDS[@]} > 0 )) && { 
    printf '%s%s\n\n  %s\n\n' \
      "${FUNC_NAME}" \
      "${LANG_MESSAGES[missing_commands]//'{NL}'/$'\n'}" \
      "${NOT_COMMANDS[*]}" 1>&2
    exit 1
  }
...
check_package() (
  ...
  (( ${#NOT_PKGNAMES[@]} > 0 )) && { 
    printf '\n%s%s\n\n  %s\n\n' \
      "${FUNC_NAME}" \
      "${LANG_MESSAGES[erroneous_names]//'{NL}'/$'\n'/}" \
      "${NOT_PKGNAMES[*]}" 1>&2
    exit 1
  }
  ...
  (( ${#NOT_INSTALLED[@]} > 0 )) && { 
    printf '\n%s%s\n\n  %s\n\n' \
      "${FUNC_NAME}" \
      "${LANG_MESSAGES[missing_packages]//'{NL}'/$'\n'}" \
      "${NOT_INSTALLED[*]}" 1>&2
    exit 1
  }
